 ...
 */
class pdf_normale_of extends ModelePDFFactures
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->page_hauteur = $formatarray['height']-40;
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->facture->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				$this->tab_top_newpage = (!getDolGlobalInt('MAIN_PDF_DONOTREPEAT_HEAD') ? 65 + $top_shift : 10);
				...
				if ($notetoshow) {
					...
					$pdf->SetFont('', 'B,U', $default_font_size);
					$pdf->SetDrawColor(255, 255, 255);
						$pdf->writeHTMLCell(190, 0,  $this->marge_gauche, $this->tab_top,  dol_htmlentitiesbr($notetoshow),0,1,false, true, 'C' );
					// Description
				...
				if (($deja_regle || $amount_credit_notes_included || $amount_deposits_included) && !getDolGlobalString('INVOICE_NO_PAYMENT_DETAILS')) {
					// $posy = $this->drawPaymentsTable($pdf, , $posy, $outputlangs);
				}
	...
	protected function drawTotalTable(&$pdf, $object, $deja_regle, $posy, $outputlangs, $outputlangsbis)
	{
		...
		if (!getDolGlobalString('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT')) {
			...
			if (getDolGlobalString('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL') && $tvaisnull) {
				...
				$pdf->MultiCell($col2x - $col1x, $tab2_hl, 'Total '.$taxe , $useborder, 'L', 1);

				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($largcol2, $tab2_hl, price($sign * $total_ttc, 0, $outputlangs)." F CFA", $useborder, 'R', 1);

				$converter = new NumberToLetter($sign * $total_ttc);
				$keidy = $converter->convert($sign * $total_ttc);
				
				$pdf->SetXY(10, $tab2_top+15 + $tab2_hl * $index);
				$pdf->SetFont('', 'B', $default_font_size+1	);
				if($total_ttc == 0){
					$pdf->SetFont('', 'B', $default_font_size);
					$pdf->SetTextColor(255, 0, 0);
					$pdf->Cell(190, $tab2_hl, "GRATUIT", 0, 1, 'C');

					$pdf->SetFont('', '', $default_font_size);
					$pdf->SetTextColor(0, 0, 0);
					$pdf->MultiCell(190, $tab2_hl,  " Offert par le Gérant, Monsieur NIARE AROUNA ", 'C', 1);
				}else{
					
					$pdf->MultiCell(190, $tab2_hl,  " Arrêté la présente facture à la somme de : ".strtoupper($keidy)." (".price($sign * $total_ttc, 0, $outputlangs).")  francs CFA ".$taxe."", 'R', 1);
				...
				$pdf->MultiCell(33, $tab2_hl, " Le Fournisseur", 'R', 1);
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '', $outputlangsbis = null)
	{
		...
		if (empty($hidetop)) {
			...
			$conf->global->MAIN_PDF_TITLE_BACKGROUND_COLOR='65,137,255';
			if (getDolGlobalString('MAIN_PDF_TITLE_BACKGROUND_COLOR')) {
				$pdf->Rect($this->marge_gauche, $tab_top, $this->page_largeur - $this->marge_droite - $this->marge_gauche, $this->tabTitleHeight+1, 'F', null, explode(',', getDolGlobalString('MAIN_PDF_TITLE_BACKGROUND_COLOR')));
			}
		}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		if (!getDolGlobalInt('PDF_DISABLE_MYCOMPANY_LOGO')) {
			if ($this->emetteur->logo) {
				...
				if (is_readable($logo)) {
					...
					$pdf->Image($logo, $this->marge_gauche, $posy, 190, 30); // width=0 (auto)
				} else {
					$pdf->SetTextColor(200, 0, 0);
		...
		$pdf->SetTextColor(0, 0, 60);
		$title = $outputlangs->transnoentities("PdfInvoiceTitle")." N° ";
		if ($object->type == 1) {
			$title = $outputlangs->transnoentities("InvoiceReplacement")." N° ";
		}
		if ($object->type == 2) {
			$title = $outputlangs->transnoentities("InvoiceAvoir")." N° ";
		}
		if ($object->type == 3) {
			$title = $outputlangs->transnoentities("InvoiceDeposit")." N° ";
		}
		if ($object->type == 4) {
			$title = $outputlangs->transnoentities("InvoiceProForma")." N° ";
		}
		...
		if ($showaddress) {
			...
			if (!getDolGlobalString('MAIN_PDF_NO_RECIPENT_FRAME')) {
				...
				// $pdf->RoundedRect($posx - 80, $posy + 15, 90, 20, $radius, '1234'); // '1234' means all four corners rounded
			}
			
			// Show recipient name
			$pdf->SetXY(22, $posy+24);
			$pdf->SetFont('', 'B', $default_font_size);
			// @phan-suppress-next-line PhanPluginSuspiciousParamOrder
			// $pdf->MultiCell(180 - 2, 2, "Doit : ".$carac_client_name, 0, $ltrdirection);
			$pdf->SetFont('', 'B,U',$default_font_size);
			$pdf->Write(2, "Doit");
			$pdf->SetFont('','B'); // Reset to normal
			$pdf->Write(2, " : " . $carac_client_name);
	...
	protected function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0, $heightforqrinvoice = 0)
	{
		...
		return pdf_pagefoot($pdf, $outputlangs, '', $this->emetteur, $heightforqrinvoice + $this->marge_basse, $this->marge_gauche, $this->page_hauteur, $object, $showdetails, $hidefreetext, $this->page_largeur, $this->watermark);
	}
