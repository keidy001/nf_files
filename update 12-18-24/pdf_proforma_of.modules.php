...
// require_once DOL_DOCUMENT_ROOT.'/core/lib/number.lib.php';

/**
 *	Class to generate PDF orders with template Eratosthene
 */
class pdf_proforma_of extends ModelePDFCommandes
{
	/**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if (getMultidirOutput($object)) {
			...
			if (file_exists($dir)) {
				...
				// $this->heightforinfotot = 50 + (4 * $nbpayments);
				...
				$tab_top_newpage = (!getDolGlobalInt('MAIN_PDF_DONOTREPEAT_HEAD') ? 65 + $top_shift : 10);
	...
	protected function drawTotalTable(&$pdf, $object, $deja_regle, $posy, $outputlangs, $outputlangsbis = null)
	{
		...
		if (!getDolGlobalString('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT')) {
			...
			if (getDolGlobalString('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL') && $tvaisnull) {
				...
				$pdf->MultiCell($largcol2, $tab2_hl, price( $total_ttc, 0, $outputlangs)." F CFA", $useborder, 'R', 1);

				$converter = new NumberToLetter( $total_ttc);
				$keidy = $converter->convert( $total_ttc);
				...
				if($total_ttc == 0){
					...
					$pdf->MultiCell(190, $tab2_hl,  " Arrêté la présente facture proforma à la somme de : ".strtoupper($keidy)." (".price( $total_ttc, 0, $outputlangs).") francs CFA ".$taxe."", 'R', 1);

				}
				$pdf->SetXY(10, $tab2_top+30 + $tab2_hl * $index);
				$pdf->MultiCell(30, $tab2_hl, " Pour Accord", 'R', 1);
				$pdf->SetXY($col2x, $tab2_top+30 + $tab2_hl * $index);
				$pdf->MultiCell(33, $tab2_hl, " Le Fournisseur", 'R', 1);
				// $pdf->SetXY($posx, $posy + 10); // Adjust position
				// $pdf->MultiCell(190, 4, $langs->trans('AmountInWords').': '.$total_ttc, 0, 'L');
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null, $titlekey = "InvoiceProForma")
	{
		...
		$title .= ' N° '.$outputlangs->convToOutputCharset($object->ref);
		...
		*/
		...
		if ($showaddress) {
			...
			$posy = getDolGlobalInt('MAIN_PDF_USE_ISO_LOCATION') ? 40 : 42;
			$posy += $top_shift+25;
			$posx = $this->page_largeur - $this->marge_droite - $widthrecbox -80;
			if (getDolGlobalInt('MAIN_INVERT_SENDER_RECIPIENT')) {
				$posx = $this->marge_gauche;
			}
			...
			// 	$pdf->SetFont('', '', $default_font_size - 2);
			// 	$pdf->SetXY($posx + 2, $posy+3);
			// 	$pdf->MultiCell($widthrecbox, 5, 'Client : ', 0, $ltrdirection);
			// 	$pdf->Rect($posx, $posy, $widthrecbox+20, 18);
			// }
			if (!getDolGlobalString('MAIN_PDF_NO_RECIPENT_FRAME')) {
				$pdf->SetTextColor(0, 0, 0);
				$pdf->SetFont('', '', $default_font_size - 2);
				...
				// $pdf->RoundedRect($posx, $posy-6, 90, 20, $radius, '1234'); // '1234' means all four corners rounded
			}
			// Show recipient name
			$pdf->SetXY(22, $posy );
			$pdf->SetFont('', 'B', $default_font_size);
