 ...
 */
class pdf_bon_livraison_of_2 extends ModelePdfExpedition
{
	/**
			...
			if (file_exists($dir)) {
				...
				if (!empty($notetoshow) || !empty($object->tracking_number)) {
					...
					//	$pdf->writeHTMLCell(190, 3, $this->posxdesc - 1, $tab_top, dol_htmlentitiesbr($notetoshow), 0, 1);
						...
						$pdf->SetDrawColor(255, 255, 255);
							$pdf->writeHTMLCell(190, 2,  $this->marge_gauche, $this->tab_top,  dol_htmlentitiesbr($notetoshow),0,1,false, true, 'C' );
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					if ($this->getColumnStatus('qty_shipped')) {
						$this->printStdColumnContent($pdf, $curY, 'qty_shipped', '');
						$nexY = max($pdf->GetY(), $nexY);
					}
	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if ($this->getColumnStatus('desc')) {
			$this->printStdColumnContent($pdf, $tab2_top, 'desc', 'Le Fournisseur');
		}
		...
		if ($this->getColumnStatus('qty_asked') && $totalOrdered) {
			// $this->printStdColumnContent($pdf, $tab2_top, 'qty_asked', 'Le Réceptionnaire');
		}

		if ($this->getColumnStatus('qty_shipped') && $totalToShip) {
			$this->printStdColumnContent($pdf, $tab2_top, 'qty_shipped', 'Le Réceptionnaire');
		}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY(40, $posy+35);
		$pdf->SetTextColor(0, 0, 60);
		$title ="BORDEREAU DE LIVRAISON N° : ".$object->ref;
		$pdf->Cell($w, 4, $title, '', 'C');
		...
		if (!empty($object->date_delivery)) {
			$posy += 6;
			$pdf->SetXY($posx-35, $posy);
			$pdf->SetTextColor(0, 0, 60);
			$pdf->MultiCell($w, 4, "Date : ".dol_print_date($object->date_delivery, "day", false, $outputlangs, true), '', 'R');
		}
		...
		if (isModEnabled($origin)) {     // commonly $origin='commande'
			...
			if ($result >= 0) {
				...
				$Yoff = $Yoff + 8;
				$pdf->SetXY($this->page_largeur - $this->marge_droite - $w-12, $Yoff);
				// $pdf->MultiCell($w, 2, $outputlangs->transnoentities("RefOrder")." : ".$outputlangs->transnoentities($text), 0, 'R');
				$Yoff = $Yoff + 6;
				$pdf->SetXY($this->page_largeur - $this->marge_droite - $w-10, $Yoff);
