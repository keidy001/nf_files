 ...
 */
class pdf_proforma_particulier extends ModelePDFCommandes
{
	/**
	...
	public function __construct(DoliDB $db)
	{
		...
		$this->db = $db;
		$this->name = "proforma_particulier";
		...
		if (!getDolGlobalString('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT')) {
			...
			if (getDolGlobalString('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL') && $tvaisnull) {
				...
				if($total_ttc == 0){
					...
					$pdf->MultiCell(190, $tab2_hl,  " Arrêté la présente facture proforma à la somme de : ".strtoupper($keidy)." F CFA ( ".price( $total_ttc, 0, $outputlangs)." F CFA TTC)", 'R', 1);
