...
if ($action != 'export_csv') {
	...
	if (getDolGlobalString('ACCOUNTANCY_SHOW_OPENING_BALANCE')) {
		...
		$sql .= " AND t.doc_date >= '".$db->idate($fiscal_period_start)."'";
	...
	foreach ($object->lines as $line) {
		...
		if($type == 'sub') {
			$opening_balance = isset($opening_balances["'".$line->subledger_account."'"]) ? $opening_balances["'".$line->subledger_account."'"] : 0;
