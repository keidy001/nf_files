...
if ($search_date_start != null) {
	$init_date = (new DateTime())->setTimestamp($search_date_start);
	$fiscal_period_start = $sql = "SELECT date_start FROM ".MAIN_DB_PREFIX."accounting_fiscalyear  WHERE date_start <= '".$init_date->format('Y-m-d')."' 
	  AND date_end >= '".$init_date->format('Y-m-d')."' ORDER BY date_start ASC LIMIT 1";
	$res = $db->query($sql);
	if ($res) { $obj = $db->fetch_object($res);
	if ($obj) { $fiscal_period_start = $obj->date_start; } } 
}
...
while ($i < min($num, $limit)) {
	...
	if ($type == 'sub') {
		...
		$accountgS = $line->subledger_account;
	} else {
		$accountg = length_accountg($line->numero_compte);
		$accountgS = $line->numero_compte;
	}
	//if (empty($accountg)) $accountg = '-';

			if(!empty($fiscal_period_start) && !empty($init_date)) {
				
		
			$keidy_sql_old_sold = 'SELECT SUM(bk.debit) as previoustotalDebit, SUM(bk.credit) as previoustotalCredit';
			$keidy_sql_old_sold .= " FROM ".MAIN_DB_PREFIX."accounting_bookkeeping as bk";
			if($type == 'sub') {
				$keidy_sql_old_sold .= " WHERE bk.subledger_account = '".$accountgS."'";
			}else {
				$keidy_sql_old_sold .= " WHERE bk.numero_compte = '".$accountgS."'";
			}
			 // Sécurisation
			// $sqlforbalance .= " AND bk.rowid < ".intval($line->id); // Sécurisation
			$keidy_sql_old_sold .= " AND bk.doc_date >= '".$db->idate($fiscal_period_start)."'";
			$keidy_sql_old_sold .= " AND bk.doc_date < '".$db->idate($init_date->format('Y-m-d'))."'";
			
			$keidy_res_old_sold = $db->query($keidy_sql_old_sold);
				// var_dump($keidy_sql_old_sold);
				// exit;
			if (!$keidy_res_old_sold) {
				echo "Erreur SQL : " . $db->lasterror();
				exit;
			}
		}
		$obj = $db->fetch_object($keidy_res_old_sold);
		$previoustotalDebit = $obj->previoustotalDebit ?: 0;
		$previoustotalCredit = $obj->previoustotalCredit ?: 0;
		$previousTotal = $previoustotalDebit - $previoustotalCredit;
	...
	if ($accountg != $displayed_account_number || !isset($displayed_account_number)) {
		...
		if (isset($displayed_account_number)) {
			...
			if(!empty($fiscal_period_start) && !empty($init_date)) {
				...
				$keidy_sql_old_sold_sub = 'SELECT SUM(bk.debit) as previousDebit, SUM(bk.credit) as previousCredit';
				$keidy_sql_old_sold_sub .= " FROM ".MAIN_DB_PREFIX."accounting_bookkeeping as bk";
				if($type == 'sub') {
					$keidy_sql_old_sold_sub .= " WHERE bk.subledger_account = '".$lastCompte."'";
				}else {
					$keidy_sql_old_sold_sub .= " WHERE bk.numero_compte ='".$lastCompte."'";
				}
				// $keidy_sql_old_sold_sub .= " WHERE bk.numero_compte = ".intval($lastCompte); // Sécurisation
				// $sqlforbalance .= " AND bk.rowid < ".intval($line->id); // Sécurisation
				$keidy_sql_old_sold_sub .= " AND bk.doc_date >= '".$db->idate($fiscal_period_start)."'";
				$keidy_sql_old_sold_sub .= " AND bk.doc_date < '".$db->idate($init_date->format('Y-m-d'))."'";
				
				$keidy_res_old_sold_sub = $db->query($keidy_sql_old_sold_sub);
				
				
				if (!$keidy_res_old_sold_sub) {
					echo "Erreur SQL : " . $db->lasterror();
					exit;
				}
			
			$objsub = $db->fetch_object($keidy_res_old_sold_sub);
			$previoustotalDebit_sub = $objsub->previousDebit ;
			$previoustotalCredit_sub = $objsub->previousCredit;
			}


			if($previoustotalDebit_sub != 0 && $previoustotalCredit_sub != 0) {
				
				print '<tr class="liste_total">';
			if ($type == 'sub') {
				print '<td class="right" colspan="' . $colspan . '">Solde Avant :</td>';
				
			} else {
				print '<td class="right" colspan="' . $colspan . '">Solde Avant :</td>';
			}
			print '<td class="nowrap right">'.price(price2num($previoustotalDebit_sub - $previoustotalCredit_sub , 'MT')).'</td>';
			...
			print '<td class="nowrap right">'.price(price2num($sous_total_debit + $previoustotalDebit_sub, 'MT')).'</td>';
			print '<td class="nowrap right">'.price(price2num($sous_total_credit+$previoustotalCredit_sub, 'MT')).'</td>';
			...
			if ($balance > 0) {
				...
				print price(price2num(($sous_total_debit - $sous_total_credit)+($previoustotalDebit_sub - $previoustotalCredit_sub), 'MT'));
				print '</td>';
				print '<td></td>';
			} else {
				print '<td></td>';
				print '<td class="nowraponall right">';
				print price(price2num(($sous_total_credit - $sous_total_debit)+($previoustotalCredit_sub - $previoustotalDebit_sub), 'MT'));
				print '</td>';
			}
			if ($colspanend > 0) {
				print '<td colspan="'.$colspanend.'"></td>';
...
if ($num > 0 && $colspan > 0) {
	if($previoustotalDebit != 0 || $previoustotalCredit != 0) {
		print '<tr class="liste_total">';
		print '<td class="right" colspan="'.$colspan.'">Solde Avant pour le compte '.$accountg.':</td>';
		print '<td class="nowrap right">'.price(price2num($previoustotalDebit - $previoustotalCredit, 'MT')).'</td>';
		print '<td class="nowrap right"></td>';
		if ($colspanend > 0) {
			print '<td colspan="'.$colspanend.'"></td>';
		}
	...
	print '<td class="nowrap right">'.price(price2num($sous_total_debit + $previoustotalDebit , 'MT')).'</td>';
	print '<td class="nowrap right">'.price(price2num($sous_total_credit + $previoustotalCredit, 'MT')).'</td>';
	if ($colspanend > 0) {
		print '<td colspan="'.$colspanend.'"></td>';
	}
	print '</tr>';
	...
	if ($balance > 0) {
		print '<td class="nowraponall right">';
		print price(price2num(($sous_total_debit - $sous_total_credit)+($previoustotalDebit - $previoustotalCredit), 'MT'));
		print '</td>';
		print '<td></td>';
	} else {
		print '<td></td>';
		print '<td class="nowraponall right">';
		print price(price2num($sous_total_credit - $sous_total_debit+($previoustotalCredit - $previoustotalDebit), 'MT'));
		print '</td>';
	}
	if ($colspanend > 0) {
		print '<td colspan="'.$colspanend.'"></td>';
...
if (!empty($totalarray['val']['totaldebit'])) {
	$totalarray['val']['totaldebit'] = (float) price2num($totalarray['val']['totaldebit']+$previoustotalDebit, 'MT');
}
if (!empty($totalarray['val']['totalcredit'])) {
	$totalarray['val']['totalcredit'] = (float) price2num($totalarray['val']['totalcredit']+$previoustotalCredit, 'MT');
}
