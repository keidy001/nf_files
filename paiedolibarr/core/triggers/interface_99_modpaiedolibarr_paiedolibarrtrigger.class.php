 ...
 * Trigger class
 */
class Interfacepaiedolibarrtrigger
{
    ...
    public function run_trigger($action, $object, $user, $langs, $conf){
        /*if($action == 'USER_MODIFY' || $action == 'USER_CREATE'){

            // print_r($_GET['url']);
            $xml = '<?xml version="1.0"?>
                <data>
                    <token>'.$object->api_key.'</token> 
                    <url>'.dol_buildpath('/',2).'</url>
                </data>
                ';
            $path = $conf->paiedolibarr->dir_output.'/'; 

            if(is_dir($path)) {
                @chmod($path, 0775);
                $file = $path.'/info_api.xml';
        ...
        if($action == 'SALARY_DELETE'){
            if($object->array_options['options_paiedolibarr_fk_paie']){
                dol_include_once('/paiedolibarr/class/paiedolibarr_paies.class.php');

                $paie = new paiedolibarr_paies($this->db);
                $res = $paie->update($object->array_options['options_paiedolibarr_fk_paie'], ['fk_salary'=>'']);
            }
        }
