...
// $arrayfields['t.old_balance']['label'] = "Solde avant";
...
//     if (!$search_account) {
//         dol_print_error(null, 'Compte non défini, mais le mode solde est activé pour le grand livre');
//         exit;
//     }

//     $sqlforbalance = 'SELECT SUM(debit - credit) as previoustotal';
//     $sqlforbalance .= " FROM ".MAIN_DB_PREFIX."accounting_bookkeeping as bk";
//     $sqlforbalance .= " JOIN ".MAIN_DB_PREFIX."accounting_account as ac ON bk.numero_compte = ac.rowid";
//     $sqlforbalance .= " WHERE ac.entity IN (".getEntity('accounting_account').")";
//     $sqlforbalance .= " AND bk.numero_compte =411 ";
//     $sqlforbalance .= " AND bk.doc_date < '".$db->idate($search_date_start)."'";

//     $resql = $db->query($sqlforbalance);
//     if ($resql) {
//         $obj = $db->fetch_object($resql);
//         $previousTotal = $obj->previoustotal ?: 0;

//         // Ajout du solde calculé au tableau
//         $arrayfields['balancebefore']['value'] = price($previousTotal);
//     } else {
//         dol_print_error($db, "Erreur lors du calcul du solde préalable du grand livre");
//         exit;
//     }
// 	        // Affichage du résultat
// 			echo '<h3>'.$langs->trans("GrandLivre").'</h3>';
// 			echo '<table class="liste">';
// 			echo '<tr class="liste_titre">';
// 			echo '<th>'.$langs->trans("Account").'</th>';
// 			echo '<th>'.$langs->trans("BalanceBefore").'</th>';
// 			echo '</tr>';
// 			echo '<tr>';
// 			echo '<td>'.$search_account.'</td>';
...
// if (!empty($arrayfields['t.balance']['checked'])) {
// 	print_liste_field_titre("Solde avant", $_SERVER['PHP_SELF'], "t.old_balance", "", $param, '', $sortfield, $sortorder, 'center ');
// }
if (!empty($arrayfields['t.debit']['checked'])) {
...
$lastLineDate = new DateTime();
$todayDate->setTime(0, 0, 0);

$sql = "SELECT date_start 
        FROM ".MAIN_DB_PREFIX."accounting_fiscalyear 
...
if ($res) {
    ...
    if ($obj) {
        $oldestFiscalYearStart = $obj->date_start;
	}
} 
...
while ($i < min($num, $limit)) {
	$line = $object->lines[$i];
	$lastLineDate = $object->lines[$i-1]->doc_date;
	$lastLine_numero_compte = $object->lines[$i-1]->numero_compte;
	$lastLineDate = (new DateTime())->setTimestamp($lastLineDate);
	// //get line date
	$line_date = $line->doc_date;
	// //convert line date  dateTime
	$line_date = (new DateTime())->setTimestamp($line_date);
	$line_date->setTime(0, 0, 0);	
	$cp_date = clone $line_date;
	$day_before = $line_date->modify('-1 day');
	// // set  line date to day before and time to 00:00:00
	$day_before->setTime(0, 0, 0);



	// // timeStampDate
	$timeStamp_day_before = $day_before->getTimestamp();
	$timeStamp_lastLineDate = $lastLineDate->getTimestamp();
	...
	if ($type == 'sub') {
		...
		// ////////////
		$numero_compte = intval($line->numero_compte); 

			// //////////// fetch solde before //////////////////////////////
			// if(!empty($search_date_start)){
			// 	$usedDate = $search_date_start;
			// }else{
			// 	$usedDate = $oldestFiscalYearStart;
			// }

			$sqlforstt = 'SELECT SUM(bk.debit) as previoustotalDebit, SUM(bk.credit) as previoustotalCredit';
			$sqlforstt .= " FROM ".MAIN_DB_PREFIX."accounting_bookkeeping as bk";
			$sqlforstt .= " WHERE bk.numero_compte = ".intval($line->numero_compte); // Sécurisation
			$sqlforstt .= " AND bk.rowid < ".intval($line->id); // Sécurisation
			$sqlforstt .= " AND bk.doc_date <= '".$db->idate($line->doc_date)."'";
			
			$resqlstt = $db->query($sqlforstt);
		...
		$objstt = $db->fetch_object($resqlstt);
		$previoustotalDebit = $objstt->previoustotalDebit ?: 0;
		$previoustotalCredit = $objstt->previoustotalCredit ?: 0;

			// var_dump($objstt);
			// exit;
			$sqlforbalance = 'SELECT SUM(bk.debit - bk.credit) as previoustotal';
			$sqlforbalance .= " FROM ".MAIN_DB_PREFIX."accounting_bookkeeping as bk";
			$sqlforbalance .= " WHERE bk.numero_compte = ".intval($line->numero_compte); // Sécurisation
			$sqlforbalance .= " AND bk.rowid < ".intval($line->id); // Sécurisation
			$sqlforbalance .= " AND bk.doc_date <= '".$db->idate($line->doc_date)."'";
		...
		$previousTotal = $obj->previoustotal ?: 0;
		// Ajout du solde calculé au tableau
		$arrayfields['balancebefore']['value'] = $previousTotal;
	// //////////// fecth current solde /////////////////////////
	$sqlforcurrentbalance = 'SELECT SUM(bk.debit - bk.credit) as line_total';
	$sqlforcurrentbalance .= " FROM ".MAIN_DB_PREFIX."accounting_bookkeeping as bk";
	$sqlforcurrentbalance .= " WHERE bk.numero_compte = ".intval($line->numero_compte); // Sécurisation
	$sqlforcurrentbalance .= " AND bk.doc_date <= '".$db->idate($line->doc_date)."'";
	// $sqlforcurrentbalance .= " AND bk.id < '".intval($line->id);
	$resqlcr = $db->query($sqlforcurrentbalance);
  
	$objdb = $db->fetch_object($resqlcr);
	$current_line_total = $objdb->line_total ?: 0;

}
	// var_dump($line);
	// var_dump($current_line_total);
	// var_dump($previousTotal);
	// exit;
	// Ajout du solde calculé au tableau
	// $arrayfields['balancebefore']['value'] = $previousTotal;
	///////////////////////////////////////////////////////////
	...
	// if (!empty($arrayfields['balancebefore']['value'] )) {
	// 	$colspan++;
	// }
	...
	if ($accountg != $displayed_account_number || !isset($displayed_account_number)) {
		...
		if (isset($displayed_account_number)) {
			...
			print '<td class="nowrap right">'.price(price2num($sous_total_debit+$previousTotal, 'MT')).'</td>';
			print '<td class="nowrap right">'.price(price2num($sous_total_credit+$previousTotal, 'MT')).'</td>';
			if ($colspanend > 0) {
	...
	if (!empty($arrayfields['t.piece_num']['checked'])) {
		...
		// Journal code
		if (!empty($arrayfields['t.code_journal']['checked'])) {
				print '<td class="center tdoverflowmax80"></td>';
			if (!$i) {
				// $totalarray['nbfield']++;
			}
		}
	...
	if (!empty($arrayfields['t.label_operation']['checked'])) {
		// Affiche un lien vers la facture client/fournisseur
			...
			print '<td class="small  classfortooltip"></td>';
	...
	// Amount debit
	if (!empty($arrayfields['t.debit']['checked'])) {
		print '<td class="right nowraponall amount"> Solde du '.$day_before->format('d-m-Y').'   : ' .($arrayfields['balancebefore']['value'] != 0 ?price(price2num($previousTotal, 'MT')):'0').'</td>';
	...
	// Amount credit
	if (!empty($arrayfields['t.credit']['checked'])) {
		print '<td class="right nowraponall amount"> </td>';
		if (!$i) {
			// $totalarray['nbfield']++;
		}
	...
	if (!empty($arrayfields['t.import_key']['checked'])) {
		print '<td class="tdoverflowmax125"></td>';
		if (!$i) {
			// $totalarray['nbfield']++;
		}
	}
	...
	if (!empty($arrayfields['t.lettering_code']['checked'])) {
		...
		// if (!empty($arrayfields['t.balance']['checked'])) {
		// 	// print '<td class="right nowraponall amount">'.price(price2num($previousTotal, 'MT')).'</td>';
		// 	if (!$i) {
		// 		$totalarray['nbfield']++;
		// 	}
		// 	if (!$i) {
		// 		// $previousTotal['pos'][$previousTotal['nbfield']] = 'totalbalance';
		// 	};
		// 	//  $previousTotal += ($line->debit - $line->credit)+($sqlforbalance);
		// }
	// Amount debit
	if (!empty($arrayfields['t.debit']['checked'])) {
	...
	if (!empty($arrayfields['t.balance']['checked'])) {
		...
		print '<td class="right nowraponall amount">'.price(price2num((($line->debit  - $line->credit)+$previousTotal), 'MT')).'</td>';
		if (!$i) {
			$totalarray['nbfield']++;
		}
		if (!$i) {
		...
		// $totalarray['val']['totalbalance'] += $sqlforbalance;
	}
	...
	$previoustotalDebit += $line->debit;
	$previoustotalCredit += $line->credit;
	print "</tr>\n";
...
if ($num > 0 && $colspan > 0) {
	...
	print '<td class="nowrap right">'.price(price2num($previoustotalDebit, 'MT')).'</td>';
	print '<td class="nowrap right">'.price(price2num($previoustotalCredit, 'MT')).'</td>';
	if ($colspanend > 0) {
		print '<td colspan="'.$colspanend.'"></td>';
	}
	print '</tr>';
	...
	if ($balance > 0) {
		print '<td class="nowraponall right">';
		print price(price2num($current_line_total, 'MT'));
...
if (!empty($totalarray['val']['totaldebit'])) {
	$totalarray['val']['totaldebit'] = (float) price2num($previoustotalDebit, 'MT');
}
if (!empty($totalarray['val']['totalcredit'])) {
	$totalarray['val']['totalcredit'] = (float) price2num($previoustotalCredit, 'MT');
}
