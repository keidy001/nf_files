...
while ($i < min($num, $limit)) {
	...
	if (!empty($arrayfields['t.code_journal']['checked'])) {
		...
		print '<td class="center tdoverflowmax100">'.$journaltoshow.'</td>';
		if (!$i) {
			$totalarray['nbfield']++;
		}
	}
	...
	// Label operation
	if (!empty($arrayfields['t.label_operation']['checked'])) {
		print '<td class="small " title="'.dol_escape_htmltag($line->label_operation).'">'.dol_escape_htmltag($line->label_operation).'</td>';
		if (!$i) {
			$totalarray['nbfield']++;
		}
	}
